...
> **Mutation Score** = Sum(抓到的bug分值) / Sum(所有bug分值) x 100%
>
...
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 1 | 老王 | CHALLENGE | 92.3% | 夯 | 23/25 | RB-WE, SS-CONN |
| 2 | Auto-I2C | AUTO | 100% | 夯 | 5/5 | - |
| 3 | 小李 | CHALLENGE | 68.5% | 中 | 17/25 | FSM-RST, IF-SEQ, ... |
...
RTL_BUG_INJECT/
├── engines/
│   ├── rtl_mutator.py          # 核心变异引擎（20种算子 + CLI + API）
...
> "嘿，你不是总说验证写的 testbench 太弱吗？来，这里有个真正的挑战——5 个 bug 藏在 I2C slave 里，你能用 debug 全找出来吗？计时开始"
>
> **他终于有大展身手的机会了。**

### 发给你的验证主管

> "老大，我们的验证流程 Mutation Score 是 92%，夯级。这是量化数据，不是拍脑袋。附 HTML 报告。"
>
> **数据说话，升职加薪。**
...
```
python engines/batch_challenger.py auto --rtl your_design.sv --tb your_tb.sv --challenge-name dare_to_test
```
